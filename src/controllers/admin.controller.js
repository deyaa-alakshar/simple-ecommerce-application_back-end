...
export const createProduct = async (req, res) => {
  try {
    ...
    const uploadResulst = await Promise.all(uploadPromises);
    const image_urls = uploadResulst.map((result) => result.secure_url);
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length === 0) {
      ...
      const uploadResulst = await Promise.all(uploadPromises);
      product.images = uploadResulst.map((result) => result.secure_url);
      await product.save();
    ...
    console.log("Error updating the products", error);
    res.status(500).json({ message: "Internal server error" });
  }
};
